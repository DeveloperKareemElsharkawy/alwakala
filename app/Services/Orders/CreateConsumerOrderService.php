...
class CreateConsumerOrderService
{
    ...
    public function createOrder($currentShoppingCarts, $request)
    {
        ...
        if ($currentShoppingCarts['cart']['address_id']) {
            $address = AddressHelper::getAddressById($request['user_id'], $currentShoppingCarts['cart']['address_id'])->toArray();
            unset($address['id']);
        ...
        $order->number = 'S-' . $request['user_id'] . '-' . time();
        $order->save();
        $order->status()->insert(['order_id' => $order->id, 'order_status_id' => 1]);


        $cartsItems = collect($currentShoppingCarts['stores'])->flatten()->pluck('items')->collapse(); // Collect Cart Items We use $cartItems So We Can Get Price From Cart

        foreach ($cartsItems as $ShoppingCart) {

            $productStoreStock = ProductStoreStock::where([
                'product_store_id' => $ShoppingCart->product_store_id,
                'color_id' => $ShoppingCart->color_id,
                'size_id' => $ShoppingCart->size_id,
            ])->first();


            if ($ShoppingCart->quantity > $productStoreStock->available_stock) {
            ...
            $orderProduct->size_id = $ShoppingCart->size_id;
            $orderProduct->store_id = $ShoppingCart->store_id;
            $orderProduct->quantity = $ShoppingCart->quantity;
            $orderProduct->purchased_item_count = $orderProduct->quantity;
            $orderProduct->item_price = $ShoppingCart->item_price;
            $orderProduct->total_price = $ShoppingCart->sub_total;
            $orderProduct->save();


            $product_order_unit_details = new ModelsProductOrderUnitDetails();
            $product_order_unit_details->order_product_id = $orderProduct->id;
            $product_order_unit_details->size_id = $ShoppingCart->size_id;
            $product_order_unit_details->color_id = $ShoppingCart->color_id;
            $product_order_unit_details->quantity = $ShoppingCart->quantity;
            $product_order_unit_details->save();

            $productStoreStock->available_stock = $productStoreStock->available_stock - $ShoppingCart->quantity;
            $productStoreStock->save();
        }

        CartItem::query()->where('user_id', $request->user_id)->delete();
        return ['status' => true];
    }
