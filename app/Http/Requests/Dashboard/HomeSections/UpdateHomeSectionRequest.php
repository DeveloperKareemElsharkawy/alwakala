...
class UpdateHomeSectionRequest extends FormRequest
{
    ...
    {
            $selected_orders=array_values(HomeSection::query()->select('order')->where('app_type',request()->app_type)->where('activation',true)->where('id','!=',request()->id)->get()->pluck('order')->toArray());
