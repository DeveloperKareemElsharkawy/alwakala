...
class FiltersController extends BaseController
{
    ...
    public function getFilters(Request $request)
    {
        $response = [
            ...
            'colors' => $this->colorsService->getColorsForSelection($request),
            'shipment_methods' => ShippingMethod::query()->where('activation', 1)
                ->select('id', 'name_' . $this->lang . ' as name')->get(),
            'policies' => Policy::query()->select('id', 'name_' . $this->lang . ' as name')->get(),
            'services' => [
                'Free Shipping'
            ],
        ];
